...
`core/middleware.ts` composes a pipeline: `withAuth → withMakeswift → withIntl → withAnalyticsCookies → withChannelId → withRoutes`. Each middleware lives under `core/middlewares/`; if you add one, register it in `middleware.ts` and consider its order — auth and Makeswift must run before locale/route resolution.
